...
const PROJECTS: ProjectItem[] = [
  ...
  },
  {
    ...
    highlight: "Top 400 / 40,000+ – PromptWars Hackathon",
    details: "Built for PromptWars Virtual (Google for Developers & Hack2Skill), ranking in the Top 400 Leaderboard out of 40,000+ global participants. Designed to optimize event operations during massive football matches with real-time stadium APIs, volunteer routing algorithms, and a Groq-powered AI assistant."
  },
  {
    title: "AMA System",
    ...
    <section className="relative z-20 bg-transparent py-16 md:py-32 px-6 md:px-12 lg:px-24 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex justify-center mb-12 sm:mb-16 px-2">
          <div
            className={`p-1 sm:p-1.5 rounded-full flex gap-1 border border-white/10 relative z-10 max-w-full overflow-x-auto ${isLowEnd ? 'bg-[#222]' : 'bg-[#1a1a1a]/40 backdrop-blur-md'}`}
            onMouseLeave={() => setHoveredTab(null)}
          >
            <button
              onClick={() => setActiveTab("featured")}
              onMouseEnter={() => setHoveredTab("featured")}
              className={`relative px-4 py-2 sm:px-6 sm:py-2.5 rounded-full text-xs sm:text-sm font-semibold tracking-wide cursor-pointer transition-colors duration-300 whitespace-nowrap ${activeTab === "featured" ? "text-white" : "text-white/60 hover:text-white"
                }`}
            >
              {activeTab === "featured" && (
                <motion.span
            ...
            <button
              ...
              className={`relative px-4 py-2 sm:px-6 sm:py-2.5 rounded-full text-xs sm:text-sm font-semibold tracking-wide cursor-pointer transition-colors duration-300 whitespace-nowrap ${activeTab === "systems" ? "text-white" : "text-white/60 hover:text-white"
                }`}
            >
              {activeTab === "systems" && (
                <motion.span
        ...
        <div className="w-full">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              layout
              className={`grid gap-6 sm:gap-8 w-full transition-all duration-500 ease-out ${isDetailedMode ? "grid-cols-1 max-w-4xl mx-auto" : "grid-cols-1 md:grid-cols-2"
                }`}
            >
              {currentProjects.map((project) => {
                return (
                  <motion.div
                    ...
                    className={`group relative overflow-hidden rounded-[1.5rem] sm:rounded-[2rem] bg-[#141414] border border-white/5 flex flex-col cursor-pointer hover:bg-[#1a1a1a] hover:border-white/10 transition-all duration-500 ${isDetailedMode ? "md:flex-row ring-1 ring-emerald-500/20 shadow-2xl shadow-emerald-950/20" : ""
                      }`}
                  >
                    {/* Image Section */}
                    <motion.div
                      ...
                      className={`relative overflow-hidden bg-black aspect-video shrink-0 transition-all duration-500 ${isDetailedMode
                          ? "w-full md:w-1/2 md:aspect-auto md:min-h-[350px] rounded-t-[1.5rem] sm:rounded-t-[2rem] md:rounded-l-[2rem] md:rounded-tr-none"
                          : "w-full rounded-t-[1.5rem] sm:rounded-t-[2rem]"
                        }`}
                    >
                      <div
                      ...
                      />
                      {/* Hover Overlay */}
                      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 sm:p-6">
                        <div className="flex items-center gap-2 text-white font-medium bg-emerald-500/90 backdrop-blur-sm px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full self-start shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 text-xs sm:text-sm">
                          <span>{isDetailedMode ? "Hide Details" : "View Details"}</span>
                          <ArrowUpRight className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-300 ${isDetailedMode ? "rotate-45" : ""}`} />
                        </div>
                      </div>
                    </motion.div>
                    ...
                    <motion.div
                      layout
                      className={`flex flex-col justify-between p-5 sm:p-6 md:p-8 flex-grow w-full ${isDetailedMode ? "md:w-1/2" : ""
                        }`}
                    >
                      <div>
                        {/* Category & Badge */}
                        <div className="flex flex-wrap justify-between items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                          <span className="text-[11px] sm:text-xs font-semibold text-emerald-400 uppercase tracking-widest">
                            {project.category}
                          </span>

                          {project.highlight && (
                            <div className="flex items-center gap-1.5 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 text-[11px] sm:text-xs font-medium">
                        ...
                        <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-6 sm:mb-8">
                          {project.tags.map((tag) => (
                            <span
                              key={tag}
                              className="px-2.5 py-0.5 sm:px-3 sm:py-1 bg-white/5 text-white/80 text-[11px] sm:text-xs rounded-full border border-white/10 hover:bg-white/10 hover:text-white transition duration-200"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      {/* Action Buttons */}
                      <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 sm:gap-4 mt-auto pt-4 border-t border-white/5">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setIsDetailedMode(!isDetailedMode);
                          }}
                          className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 sm:px-5 sm:py-2.5 bg-white/5 hover:bg-white/10 text-white font-medium text-xs sm:text-sm rounded-xl border border-white/10 transition duration-300 cursor-pointer"
                        >
                          <span>{isDetailedMode ? "Hide Details" : "View Details"}</span>
                          <ArrowUpRight className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-300 ${isDetailedMode ? "rotate-45" : ""}`} />
                        </button>
                        ...
                        {project.link && (
                          <a
                            ...
                            className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 sm:px-5 sm:py-2.5 bg-emerald-500 hover:bg-emerald-400 text-[#0f0f0f] font-semibold text-xs sm:text-sm rounded-xl transition duration-300 shadow-lg shadow-emerald-500/10"
                          >
