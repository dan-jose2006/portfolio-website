    ...
    <>
      <div className="fixed top-0 left-0 w-full z-50 flex justify-between items-center p-4 md:p-8 pointer-events-none">
        ...
        >
          ...
          <button
            className={`lg:hidden flex items-center justify-center p-2 md:p-2.5 text-white rounded-full transition-colors ${isLowEnd ? 'bg-[#333]' : 'bg-white/5 border border-white/10 backdrop-blur-md shadow-lg hover:bg-white/10'}`}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className={`fixed inset-x-4 top-20 z-40 rounded-2xl overflow-hidden shadow-2xl border border-white/10 lg:hidden ${isLowEnd ? 'bg-[#222]' : 'bg-[#1a1a1a]/95 backdrop-blur-xl'}`}
          >
            <div className="flex flex-col p-4">
              {navLinks.map((link) => (
                <a
                  ...
                  className="px-6 py-4 text-white/80 font-medium text-lg hover:text-white hover:bg-white/5 rounded-xl transition-colors border-b border-white/5 last:border-0"
                >
                  {link.name}
                </a>
              ))}
              <a
                href="#contact"
                onClick={() => setIsMobileMenuOpen(false)}
                className="mt-4 px-6 py-3 bg-emerald-500 text-[#121212] font-bold text-center rounded-xl transition-colors hover:bg-emerald-400 uppercase tracking-wider text-sm"
              >
                Let&apos;s Talk
              </a>
            </div>
